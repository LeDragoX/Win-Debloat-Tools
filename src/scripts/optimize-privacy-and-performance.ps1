...
Function TweaksForPrivacyAndPerformance() {
    ...
    Write-Host "$($Status[0]) Show me the windows welcome experience after updates..."
    Write-Host "$($Status[0]) 'Get fun facts and tips, etc. on lock screen'..."
    ...
    ForEach ($Name in $ContentDeliveryManagerDisableOnZero) {
        Write-Host "$($Status[0]) $($Name): $Zero"
        Set-ItemProperty -Path "$PathToContentDeliveryManager" -Name "$Name" -Type DWord -Value $Zero
    }

    Write-Host "$($Status[0]) 'Suggested Content in the Settings App'..."
    ...
    Write-Host "$($Status[0]) 'Show Suggestions' in Start..."
    ...
    Write-Host "$($Status[0]) Let apps use my advertising ID..."
    ...
    Write-Host "$($Status[0]) 'Let websites provide locally relevant content by accessing my language list'..."
    ...
    Write-Host "$($Status[0]) Online Speech Recognition..."
    ...
    Write-Host "$($Status[0]) Tailored Experiences..."
    ...
    Write-Host "$($Status[0]) View diagnostic data..."
    ...
    Write-Host "$($Status[0]) feedback frequency"
    ...
    Write-Host "$($Status[0]) Activity History..."
    ...
    Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Services\lfsvc\Service\Configuration" -Name "Status" -Type DWord -Value $Zero
    ...
    Write-Host "$($Status[0]) Denying device access..."
    ...
    Write-Host "$($Status[0]) Automatic Download and Installation of Windows Updates..."
    ...
    Write-Host "$($Status[0]) Automatic Updates..."
    ...
    Set-ItemProperty -Path "$PathToWindowsUpdate" -Name "ScheduledInstallDay" -Type DWord -Value $Zero
    ...
    Write-Host "$($Status[1]) Assuring automatic driver update is ENABLED..."
    ...
    Write-Host "$($Status[1]) Change Windows Updates to 'Notify to schedule restart'..."
    ...
    Write-Host "$($Status[1]) Restricting Windows Update P2P downloads for Local Network only..."
    ...
    Write-Host "$($Status[1]) Automatic Recommended Troubleshooting, then notify me..."
    Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\WindowsMitigation" -Name "UserPreference" -Type DWord -Value 3

    Write-Host "$($Status[0]) Windows Spotlight Features..."
    Write-Host "$($Status[0]) Third Party Suggestions..."
    Write-Host "$($Status[0]) More Telemetry Features..."
    ...
    Write-Host "$($Status[0]) Apps Suggestions..."
    ...
    Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\MRT" -Name "DontOfferThroughWUAU" -Type DWord -Value $One
    ...
    Write-Host "$($Status[0]) 'WiFi Sense: HotSpot Sharing'..."
    ...
    Write-Host "$($Status[0]) 'WiFi Sense: Shared HotSpot Auto-Connect'..."
    ...
    Write-Host "$($Status[0]) Game Bar & Game DVR..."
    ...
    ForEach ($Name in $ExplorerAdvKeysToZero) {
        Write-Host "$($Status[1]) Setting $Name value: $Zero"
        Set-ItemProperty -Path "$PathToExplorerAdvanced" -Name "$Name" -Type DWord -Value $Zero
    }

    Write-Host "[Registry] From Path: [$PathToExplorerAdvanced]"
    ForEach ($Name in $ExplorerAdvKeysToOne) {
        Write-Host "$($Status[1]) Setting $Name value: $One"
        Set-ItemProperty -Path "$PathToExplorerAdvanced" -Name "$Name" -Type DWord -Value $One
    }

    Write-Host "$($Status[1]) Showing file transfer details..."
    ...
    Write-Host "$($Status[1]) Showing task manager details..."
    ...
    Set-ItemProperty -Path "$PathToPsched" -Name "NonBestEffortLimit" -Type DWord -Value $Zero
    Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Multimedia\SystemProfile" -Name "NetworkThrottlingIndex" -Type DWord -Value 0xffffffff

    Write-Host "[@] (2 = Disable, 4 = Enable)"
    Write-Host "$($Status[0]) Windows Store apps Automatic Updates..."
    ...
    Write-Host "$($Status[1]) (Not setting) the Ultimate Performance Power Plan..."
